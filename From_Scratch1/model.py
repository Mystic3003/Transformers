...
class PositionalEncoding(nn.Module):
    def __init__(self,d_model,seq_length,dropout:float)->None:
        super().__init__()
        self.d_model= d_model
        self.seq_length =seq_length
        self.dropout=nn.Dropout(dropout)
        ##vector of size d_model , as many as seq_length
        ...
        #Apply the sin to even positions , cos for odd positions
        pe[:,0::2]= torch.sin(position *div_term)
        pe[:,1::2] =torch.cos(position * div_term)

        pe=pe.unsqueeze(0) ##(1,seq_length ,d_model)
        self.register_buffer('pe',pe) ### parameter to be saved , not learned.

    def forward(self,x):
        x= x + (self.pe[:,:x.shape[1],:]).requires_grad_(False)
        return self.dropout(x)

##Layer Normalization x =x-mu / sqrt(variance + eps) [Add and Norm Block]
class LayerNormalization(nn.Module):
    def __init__(self, eps: float =10**-56) ->None :
        super().__init__()
        self.eps= eps
